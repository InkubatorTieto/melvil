...
def test_add_book(view_book, client):
    client.post(
        url_for("library_books.add_book"),
        data=view_book.data,
        follow_redirects=True,
    )

    author = Author.query.filter_by(
        first_name=view_book.first_name.data, last_name=view_book.surname.data
    ).first()
    ...
    assert tag.name in view_book.tag.data, "Tags ane not the same"
...
def test_add_the_same_book(view_book, client):
    ...
    assert not bool(view_book.errors), "Two same books have been added."
