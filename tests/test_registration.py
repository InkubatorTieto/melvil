...
#
#
# def test_registration_invalid_data_inserted(client, session, registration_form_invalid):
#     client.post(url_for('library.registration'),
#                        data=registration_form_invalid.data,
#                        follow_redirects=True)
#     user = User.query.filter_by(email=registration_form_invalid.email.data).first()
#     assert user is None, \
...
#
#
# def test_registration_data_of_registered_user_inserted(client, session, registration_form_registered_user):
#     client.post(url_for('library.registration'),
#                        data=registration_form_registered_user.data,
#                        follow_redirects=True)
#     user = User.query.filter_by(email=registration_form_registered_user.email.data).count()
#     assert user == 1, \
#         "Registration view, user registered twice"
#     if user > 1:
#         user = User.query.filter_by(email=registration_form_registered_user.email.data).all()
#         session.delete(user[1])
#         session.commit()
#
#
# # def test_registration_POST_no_data(app):
# #     with app.test_client() as c:
# #         try:
# #             c.post(url_for('library.registration'),
# #                        follow_redirects=True)
# #             assert False, "No error when no user data passed"
# #         except:
# #             assert True
#
