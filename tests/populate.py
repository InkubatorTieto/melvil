...
from models import RentalLog, Copy, Book, Author, Tag, User, Magazine
...
def populate_authors(n=40):
    return [
        Author(first_name=g.person.name(), last_name=g.person.last_name())
        for _ in range(n)
    ]


def populate_tags(n=15):
    return [Tag(name=g.text.word()) for _ in range(n)]
