...
class Copy(db.Model):
    ...
    library_item_id = db.Column(
        db.Integer, db.ForeignKey("library_item.id"), nullable=False
    )
    ...
    def __repr__(self):
        return "<Copy: {} library_item_id={}>".format(
            self.asset_code, self.library_item_id
        )
...
class RentalLog(db.Model):
    ...
    def borrow_time(self):
        return self._borrow_time.replace(tzinfo=pytz.utc).astimezone(
    ...
    def return_time(self):
        return self._return_time.replace(tzinfo=pytz.utc).astimezone(
            tz=pytz.timezone("Europe/Warsaw")
        )
    ...
    def reservation_begin(self):
        return self._reservation_begin.replace(tzinfo=pytz.utc).astimezone(
            tz=pytz.timezone("Europe/Warsaw")
        )
    ...
    def reservation_end(self):
        return self._reservation_end.replace(tzinfo=pytz.utc).astimezone(
            tz=pytz.timezone("Europe/Warsaw")
        )
    ...
    def __str__(self):
        return "RENTAL LOG: user: {} copy: {}".format(
            self.user.full_name, self.copy.asset_code
        )

    def __repr__(self):
        return "<RentalLog: user_id={} copy_id={}>".format(
            self.user_id, self.copy_id
        )
